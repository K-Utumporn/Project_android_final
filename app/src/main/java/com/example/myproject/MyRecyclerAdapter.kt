...
import com.example.myproject.detail_book
...
//        holder.detailTextView.setText( dataSource.getJSONObject(position).getString("description").toString() )
        ...
        holder.layout.setOnClickListener{
            ...
            val detailBook = detail_book().newInstance(title, detail, img)
            val fm = thisActivity.supportFragmentManager
            val transaction: FragmentTransaction = fm.beginTransaction()
            transaction.replace(R.id.layout, detailBook, "fragment_detail_book")
